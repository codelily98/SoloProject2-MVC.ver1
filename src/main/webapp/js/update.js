$(document).ready(function() {
	...
	$("#resetbtn").click(function(){
		window.location.href = "./infoForm.jsp";
	});
	...
	$('#pwd').on('blur', function() {
	    ...
	    $.ajax({
	        url: '../member/checkPwd.jsp',  // 서버의 중복 체크 URL
	        method: 'POST',
	        data: {
				id: id, 
				pwd: pwd 
			...
			success: function(data) {
			    let result = data.exists; // 데이터를 직접 확인
				console.log("나 비번 체크 : " + result);
    ...
    $('#updatebtn').on('click', function(e) {
        ...
        if ($('#checkinput li').length === 0) {
            ...
            $.ajax({
                url: '../member/update.jsp',  // 서버의 회원가입 처리 URL
                method: 'POST',
                data: formData,
				dataType: 'text',
				success: function(data) {
